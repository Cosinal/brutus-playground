...
export function GrowthMarketComparisonChart({ id }: ChartProps) {
  return (
    <div id={id} className="bg-zinc-900 rounded-lg p-6 border border-zinc-800">
      ...
      <p className="text-xs text-zinc-500 mb-4">Source: Yahoo Finance as of 2026-08-26 | Market data requires live API or CSV import</p>
      <div className="p-12 text-center text-zinc-400">
        <p className="mb-4">5-year indexed comparison (EGO in USD, ELD.TO in CAD, GC=F gold futures)</p>
        <p className="text-sm mb-6">Live market data requires Yahoo Finance v8 chart API or Stooq CSV fallback.</p>
        ...
        <p className="text-xs mt-6 text-zinc-600">To implement: Fetch Yahoo v8 chart or Stooq CSV, index to 100 at start date, save static snapshot</p>
      </div>
    </div>
  );
}
